	...
	implements MigrationInterface
{
	public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      -- Mowing data query filtered by year
      ...
      FROM (
        ...
        AND e.date::DATE >= year_start
        AND e.date::DATE <= year_end
        GROUP BY
      ...
      -- Fertilizer timeline data filtered by year
      ...
      -- Product type distribution filtered by year
      ...
      -- Average days between activities filtered by year
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
      ...
      average_days_between_data JSON;
      BEGIN
      -- Fixed mowing data query with proper date comparison
      SELECT json_agg(row_to_json(m)) INTO mow_data
      FROM (
        SELECT
        EXTRACT(MONTH FROM e.date)::INT AS "month",
        ...
        e.user_id = p_user_id
        AND ea.activity_id = 1  -- Mowing
        AND e.date::DATE >= DATE_TRUNC('year', CURRENT_DATE) + INTERVAL '1 month'
      ...
      FROM (
        WITH activity_dates AS (
          ...
          WHERE e.user_id = p_user_id
            AND ea.activity_id = 1  -- Mowing activity
            AND e.date::DATE >= DATE_TRUNC('year', CURRENT_DATE)
            AND e.date::DATE <= CURRENT_DATE
